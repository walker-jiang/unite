...
export default class Index extends Component {
  ...
  getinspectionData(value){
    ...
    function success(res) {
      if(res.result){
        let inspectionItemsData = res.data.baMedicalDtlList.map((item, index)=>{
          item.key = index; // 加唯一key值
          item.status = (index == 0) ? 1 : 0; // 0表示全部未选中,1表示选择了该行,初始化时默认选中第一行
          return item
        });
        res.data.baOrderSuitList.forEach((item, index)=>{
          item.key = inspectionItemsData.length; // 加唯一key值
          inspectionItemsData.push(item);
        });
        let totalLines = inspectionItemsData.length;
        console.log('inspectionItemsData', inspectionItemsData);
        that.setState({inspectionItemsData, totalLines});
      }else{
  ...
  getEnterValue (curLine) {
    ...
    let quickAddData = inspectionItemsData[curLine];
    this.setState({
      showResult: false,
    }, function () {
      this.props.getQuickData(quickAddData)
  ...
  checkedLine(record, status){
    let inspectionItemsData = this.state.inspectionItemsData;
    inspectionItemsData.map((item)=>{ // 改变当前行的选中状态
    ...
    this.setState({ inspectionItemsData });
  };
  // 将除当前点击行外的所有行均设置为未选中
  SelectedLine(record){
    let inspectionItemsData = this.state.inspectionItemsData;
    inspectionItemsData.map((item)=>{
      if(item.status != 2){
        if(item.key == record.key){
          if(item.status == 1){
            item.status = 0;
  ...
  handleEnterPress = (e) => {
    ...
    switch(e.keyCode){
      case 40:         // 向下箭头, 选择下一行
        ...
        this.SelectedLine(inspectionItemsData[curLine]);
        break;
      case 38:         // 向上箭头，选择上一行
        if(curLine <= 0){
      ...
      case 13:         // Enter 添加到处方列表
        this.checkedLine(inspectionItemsData[curLine], 2);
  ...
  render() {
    let { showResult, inspectionItemsData } = this.state;
          ...
          (
            <Result>
              <SpecTable
                ...
                columns={columns}
                dataSource={inspectionItemsData}
                pagination={false}
              >
              </SpecTable>
            </Result>
