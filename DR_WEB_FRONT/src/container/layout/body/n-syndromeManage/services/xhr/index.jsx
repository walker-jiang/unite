...
const targetTwo = Config.targetTwo;
 ...
 * @param {function} error    请求失败执行方法
 */
Tool.postAddUrl = function (pathname, data, async) {
    console.log("POST地址为:",targetTwo+pathname);
    console.log("POST数据为:",data.BaHerbalMedicine);
    var returnResult = {}
    //可用 fetch替换
    $.ajax({
      type: 'POST', //请求的方式
      url: `${targetTwo+pathname}?orgCode=${data.orgCode}&BaHerbalMedicine=${data.BaHerbalMedicine}`,//ajax请求地址
      dataType: 'json',//数据类型
