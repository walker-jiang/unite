...
class Index extends Component {
  ...
  componentWillMount(){
    ...
    if(this.props.actionType == 'modify' || this.props.actionType == 'view'){ // 修改、查看需要初始化数据
      ...
      if(JSON.stringify(this.props.attachOrder) != '{}'){
        ...
        this.setState({
          inspectionData: buOrderDtlList.concat(buOrdmedicalSuitList),
          aim: Recipe.aim,
  ...
  getTableColumns(){
    ...
    let columns = [{
      ...
      render: (text, record)=>(
        <SpecSelect
          defaultValue={{key: record.deptcode, label: record.deptname}}
          labelInValue={true}
  ...
  render () {
    let { visiblePop, inspectionData, buDiagnosisList, mitype, selectedMitype, aim } = this.state;
            ...
            <FormItem
              ...
              >
                {getFieldDecorator('addQuickly')(
                  <QuickAddInspectionItem placeholder='请输入检查项目首字母快速添加' selectedMitype={selectedMitype} icon='#0A6ECB' ref={ref => this.quickAddExamineItem = ref} getQuickData = {this.addInspectionData.bind(this)}/>
                )}
              </FormItem>
          </Col>
        </Row>
