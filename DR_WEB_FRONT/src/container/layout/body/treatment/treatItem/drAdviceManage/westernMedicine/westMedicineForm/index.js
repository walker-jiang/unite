...
class Index extends Component {
  ...
  delWestMedicineData(record) {
    ...
    if(record.orderSuitid){
      confirm({
        ...
        WestMedicineData = WestMedicineData.remove({medicalid: record.medicalid});
        that.setState({WestMedicineData})
    }
  }
  /**
  ...
  addWestMedicineData (WestMedicineItem) {
    ...
    for(let i=0; i < WestMedicineData.length; i++){
      if(WestMedicineData[i].baMedicalDtlList){ // 医嘱套
        if(WestMedicineData[i].orderSuitid == WestMedicineItem.orderSuitid){
          this.tipModal.showModal({ stressContent: '该西医治疗项已存在' });
          return false;
        }
      }else{ // 非医嘱套
        if(WestMedicineData[i].medicalid == WestMedicineItem.medicalid){
          this.tipModal.showModal({ stressContent: '该西医治疗项已存在' });
          return false;
        }
      }
    }
    WestMedicineData.push(WestMedicineItem);
    this.setState({ WestMedicineData });
  }
          ...
          <Col span={24}>
            <FormItem
              ...
              {getFieldDecorator('choosedItem', {
                ...
                <div>
                {
                  WestMedicineData.map((item, index) => <SpecTag  onClose={(e) => {e.preventDefault();this.delWestMedicineData(item)}} closable key={index} id={item.baMedicalDtlList ? item.orderSuitid : item.medicalid}>{item.baMedicalDtlList ? item.orderSuitname : item.medicalname}</SpecTag>)
                }
                </div>
              )}
            </FormItem>
