...
const TEMPLATE_TYPE = 0;    //模板类型 医嘱 1 病历 0
...
class BLClassify extends Component {
    ...
    _handleGetList = () => {
        if (getList) {
            getList({ temtype: 0, ...DEFAULT_PARAM }).then((data) => {
                const { _currentTreeItem } = this.state;
                let _data = {}, len = 0;
    ...
    handleTableOpt = (record, mType) => {
        ...
        if (openTmp) {
            //模板:1 分类:0
            (type == "0") ? this.handleClassifyhowModal(record, mType) : openTmp('bl', { "temmanageid": record['temmanageid'] }, tmpName, mType, _currentTreeItem);
        } else { return }
    }
        ...
        return (
            <div>
                ...
                <Row>
                    ...
                    <Col span={18}>
                        <Table columns={this._tableColumn} dataSource={_tableData} rowKey="temmanageid" /></Col>
                </Row>
                {classifyModelVisiable && <ClassifyModal modalProps={{
                    ...
                    getDataFun={getList}
                    getDataParam={{ temtype: 0, ...DEFAULT_PARAM }}
