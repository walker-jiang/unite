...
export default class Index extends Component {
  ...
  addExamineItem(values, inspectionData){
    ...
    let buOrderDtlList = new Array();   // 检查项目非医嘱套
    let buOrdmedical = { // 检查项目医嘱套
      aim: values.aim,
  ...
  modifyExamineItem(values, inspectionData){
    ...
    console.log('inspectionData修改', inspectionData);
    let ordercontent = '';
    let feeall = 0;
    let self = this;
      ...
      <BasicModal visible={visiblePop} title={title + '检查单'} onClose={() => this.handlePopClose()}>
        <Wrap>
          <InspectionForm {...this.props} wrappedComponentRef={ref => {this.form = ref}}></InspectionForm>
          {
            ...
            <Footer>
              <SureButton type="primary" onClick={this.saveForm}>保存</SureButton>
